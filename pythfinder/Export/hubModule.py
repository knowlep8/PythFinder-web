...
def _actions_source(actions: list) -> str:
    """The action functions, if any, and run() -- the only thing runs.py needs.

    `actions is None` means the caller never described actions at all: the
    original one-step export (`Trajectory.hub_module(name, steps)` with no
    third argument), wired by hand into a runs.py that already has its own
    marker functions. Nothing here belongs in that file, so it stays exactly
    as empty as before. An empty *list* is different -- build_run always
    passes one, even for a run with nothing attached -- and means a real run
    that happens to have zero actions. It still needs run() to be
    self-contained; that was the whole point of step 3.1, and a run with no
    actions is not a smaller point of it, just a shorter one.
    """
    if actions is None:
        return ""
...
def _module_text(heading: str, steps: int, markers: tuple, count: int,
    ...
    return (
        ...
        "DATA = (\n{3}\n)\n"
    ).format(steps, markers, count, data_block) + _actions_source(actions)


def _heading(name: str, actions: list) -> str:
    heading = '"""{0}, generated by PythFinder -- do not edit."""\n'.format(name)

    # None means the caller never described actions at all -- see
    # _actions_source -- so there is no run() here to import Trajectory for.
    # An empty list still gets one, so it still needs the import.
